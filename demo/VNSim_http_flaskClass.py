...
class MyApp(QWidget):
    ...
    def onStartClick(self, flag_start_with_bright_eye=False):
        if self.file_name and self.controller_list:
            spawn_point = [10,10,90]
            self.wbts = WbtProcessParallel(
                self.file_name, self.controller_list, spawn_point, flag_start_with_bright_eye
            )
            self.wbts.run()
            # logger.info(f"启动完毕")
        else:
...
class WbtProcessParallel:
    def __init__(self, wbt_file, controller_list, spawn_point, flag_start_with_bright_eye=False):
        self.process = []
        ...
        print("kkkkkkkkkk",self.spawn_point,type(self.spawn_point))
    ...
    def handle_wbt_file(self, start_wbt_file, controller_type):
        ...
        for node in wp.content["root"]:
            ...
            if node["name"] == "Robot" and node["DEF"] == "RobotNode":
                ...
                has_rotation = False                     
                for field in node["fields"]:
                    # 有字段要修改值，没有字段要增加字段，角度也要处理好
                    if field["name"] == "translation":
                        field["value"][0] = str(self.spawn_point[0])
                        field["value"][1] = str(self.spawn_point[1])
                    ...
                    if field["name"] == "rotation":
                        ...
                        field["value"][1] = str(0)              
                        field["value"][2] = str(1)
                        field["value"][3] = str(math.radians(self.spawn_point[2]))
                        has_translation = True
                ...
                if has_translation == False:
                        # logger.info("没有translation字段，创建")
                        node["fields"].append(
                            {
                                "name": "translation",
                                "type": "SFVec3f",
                                "value": [str(self.spawn_point[0]),str(self.spawn_point[1]),'0'],
                            }
                        )

                if has_rotation == False:
                        # logger.info("没有rotation字段，创建")
                        node["fields"].append(
                            {
                                "name": "rotation",
                                "type": "SFRotation",
                                "value": ['0','0','1',str(math.radians(self.spawn_point[2]))],
                            }
                        )
...
class FlaskApp:
    ...
    def find_wbt_file(self,filename):
        # find wbt file by filename in /home/visionnav/vn_wbt_project/ directory
        for root, dirs, files in os.walk("/home/visionnav/vn_wbt_project/"):
            for file in files:
                if file == filename+'.wbt':
                    return os.path.join(root, file)

    def setup_routes(self):

        @self.app.route('/start_wbt', methods=['POST'])
        def start_wbt():
            data = request.json
            filename = data.get('filename')
            file_path = self.find_wbt_file(filename)
            spawn_point = data.get('spawn_point')
            print(spawn_point)
            flag_start_with_bright_eye = data.get('flag_start_with_bright_eye', False)
            return self.handle_http_start_wbt(file_path, spawn_point,flag_start_with_bright_eye)

        @self.app.route('/stop_force', methods=['POST'])
        def stop_force():
            return self.handle_http_stop_force()


    def handle_http_start_wbt(self,file_path, spawn_point,flag_start_with_bright_eye=False):

        if not file_path:
            print("您未选择文件，请选择wbt文件")
            return jsonify({"status": "error", "message": "未选择文件，请选择wbt文件"})
        elif not file_path.endswith(".wbt"):
            print("该文件不是wbt文件，请选择wbt文件")
            return jsonify({"status": "error", "message": "该文件不是wbt文件"})
        else:
            wbtc = WbtChecker(file_path)
            wbtc.get_Robot_controller_list()
            controller_list = wbtc.controller_list

            wbt_process = WbtProcessParallel(file_path, controller_list, spawn_point, flag_start_with_bright_eye)
            wbt_process.run()
            print(f"启动完毕")
            return jsonify({"status": "success", "message": "启动完毕"})

        if self.wbts is None:
            logger.warning("请先用启动器启动wbt")
            return jsonify({"status": "error", "message": "请先用启动器启动wbt"})
        else:
            try:
                self.file_name_copy = self.wbts.start_wbt_list
                self.wbts.stop_all_wbt()
            ...
            return jsonify({"status": "success", "message": "停止所有wbt"})

    def handle_http_stop_force(self):
        # 使用更加安全的命令执行方式，并确保命令执行的正确性
        try:
            # 查询webots进程，这里保持使用列表传递参数，因为没有外部输入，所以相对安全
            subprocess.run(
            ...
            if result.returncode != 0:
                # logger.error(
                #     f"未能终止webots进程，错误码：{report.returncode}。错误码：1，表示无webots进程。"
                # )
                return jsonify({"status": "fail", "message": "The process was not found."})
        except Exception as e:
            # logger.error(f"操作过程中发生错误：{e}")
            return jsonify({"status": "success", "message": "停止所有wbt进程"})
...
if __name__ == "__main__":
    ...
    # 启动Flask应用
    def run_flask_app():
        FlaskApp().app.run(host='0.0.0.0', port=5000)
    ...
    # 单独测试WbtProcess
    # handler = WbtProcess('/home/visionnav/simulate_workspace/simulation_world/webots/worlds/lgc.wbt',['E_master','E_shadow_perception','E_shadow_lidar'])
    # handler.run()
    # import time
    # time.sleep(30)
