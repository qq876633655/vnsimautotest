...
if [ ! -f "$HOST_ROOT_DIR/vn_wbt_assets.zip" ]; then
    echo "ERROR: $HOST_ROOT_DIR/vn_wbt_assets.zip not exist"
    exit 1
fi
if [ ! -d $HOST_ROOT_DIR/sim_module_pkg ]; then
	echo "ERROR: $HOST_ROOT_DIR/sim_module_pkg not exist"
	exit 1
fi
docker run -it --rm --privileged --gpus all \
	 ...
	 -v $HOST_ROOT_DIR/vn_wbt_assets.zip:$DOCK_ROOT_DIR/vn_wbt_assets.zip \
	 ...
	 --pid=host \
	 --network=host \
	 simulation:latest ./sim_module_pkg/simulate_assistant_flask